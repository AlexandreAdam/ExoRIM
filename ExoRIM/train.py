...
class Training(TrainViz, TrainMaster):
    ...
    def _epoch(self, epoch, verbose=1):
        train_loss_trace = 0  # these are tracers to compute loss, reset each self.train_trace batch
        test_loss_trace = 0
        # training
        for image in self.generator.training_batch():
            ...
            if self.generator.train_index in self.train_traces:
                ...
                if verbose > 0:
                    print(f"epoch {epoch + 1}: training loss = {train_trace_loss}")
                train_loss_trace = 0  # reset the trace
        # test
        for image in self.generator.test_batch():
            ...
            if self.generator.test_index in self.test_traces:
                self.test_losses.append(test_loss_trace)
                self.snap(output, image, state="test")
                if verbose > 0:
                    print(f"epoch {epoch + 1}: test loss = {test_loss_trace}")
                test_loss_trace = 0  # reset the trace
