...
checkpoint_path_1 = "checkpoints/models/"
...
with tf.device('/cpu:0'):
    ...
    RIM = RIM(train_batch_size, num_steps, num_features , state_size)
    ...
    for train_iter in range(train_batch_size):
        ...
        tf_IM =  tf.identity(IM_gen.IM_tr[train_iter,:,:,:].reshape((1, IM_gen.im_size, IM_gen.im_size, 1)))

        with tf.GradientTape() as tape:
            tape.watch(RIM.model_1.variables)
            cost_value, os1 = RIM.cost_function(tf_IM, noisy_data, noise_rms)
