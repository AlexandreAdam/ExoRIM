...
class ConvUpsampleBlock(tf.keras.layers.Layer):
    def __init__(self, filters, kernel_size, kernel_reg_amp=0, bias_reg_amp=0, activation="relu", alpha=0.3, **kwargs):
        super(ConvUpsampleBlock, self).__init__(DTYPE, **kwargs)
        if activation == "leaky_relu":
            self.non_linearity = tf.keras.layers.LeakyReLU(alpha=alpha)
        else:
            self.non_linearity = tf.keras.layers.Activation(activation)  # gelu, elu, relu
